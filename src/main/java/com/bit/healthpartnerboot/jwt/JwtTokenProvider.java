...
public class JwtTokenProvider {
    ...
    public String createAccessToken(String memberEmail) {
        Date accessTokenExpireDate = Date.from(Instant.now().plusMillis(ACCESS_TOKEN_EXPIRATION_MS));
        return createToken(memberEmail, accessTokenExpireDate);
    }

    public void createRefreshToken(String memberEmail) {
        Date refreshTokenExpireDate = Date.from(Instant.now().plusMillis(REFRESH_TOKEN_EXPIRATION_MS));
    ...
    public boolean isLogout(String token) {
        return logoutTokenRepository.findByAccessToken(token) != null;
    }
